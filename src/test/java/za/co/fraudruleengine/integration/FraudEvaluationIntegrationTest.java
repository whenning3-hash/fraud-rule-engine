...
class FraudEvaluationIntegrationTest {
    ...
    // CorrelationIdFilter is a @Component servlet filter; WebEnvironment.MOCK does NOT
    // auto-register @Component filters into MockMvc (only Spring Security's FilterChainProxy
    // is wired via springSecurity()).  Inject and add it explicitly so tests can assert on the
    // X-Capitec-Correlation-ID response header.
    ...
    @Test
    void submitTransaction_withCorrelationIdHeader_isEchoedInResponse() throws Exception {
        // When the caller supplies X-Capitec-Correlation-ID, the same value must be echoed
        // back in the response header so callers can correlate their trace with server logs.
        String correlationId = "integration-test-trace-abc123";
        mockMvc.perform(post("/api/v1/transactions")
                        .contentType(MediaType.APPLICATION_JSON)
                        .header("X-Capitec-Correlation-ID", correlationId)
                        .content(toJson(cleanTransaction("ACC-CORR-ECHO"))))
                .andExpect(status().isCreated())
                .andExpect(header().string("X-Capitec-Correlation-ID", correlationId));
    }

    @Test
    void submitTransaction_withoutCorrelationIdHeader_responseContainsAutoGeneratedId() throws Exception {
        // When no X-Capitec-Correlation-ID header is supplied, the filter generates a UUID
        // and writes it to the response header so callers can still trace the server-side log.
        MvcResult result = mockMvc.perform(post("/api/v1/transactions")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(toJson(cleanTransaction("ACC-CORR-AUTO"))))
                .andExpect(status().isCreated())
                .andReturn();

        String generated = result.getResponse().getHeader("X-Capitec-Correlation-ID");
        assertThat(generated).isNotBlank();
        // Must be a UUID — 36 characters with hyphens
        assertThat(generated).matches("[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}");
    }

    @Test
    void submitTransaction_created_responseIncludesLocationHeader() throws Exception {
        // RFC 7231 §6.3.2: 201 Created responses SHOULD include a Location header pointing
        // to the created resource, allowing clients to retrieve it without parsing the body.
        MvcResult result = mockMvc.perform(post("/api/v1/transactions")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(toJson(cleanTransaction("ACC-LOCATION-TEST"))))
                .andExpect(status().isCreated())
                .andReturn();

        String location = result.getResponse().getHeader("Location");
        assertThat(location).isNotBlank();
        assertThat(location).contains("/api/v1/transactions/");
    }

    @Test
    void submitTransaction_overLongAccountId_returns400WithSizeViolation() throws Exception {
        // The DB column is VARCHAR(100). An over-length value must be rejected at the API
        // boundary (Bean Validation @Size) rather than failing with a DB constraint error.
        String overLong = "A".repeat(101);
        mockMvc.perform(post("/api/v1/transactions")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(toJson(new TransactionRequest(
                                overLong, new BigDecimal("100.00"), "ZAR",
                                "Test Merchant", null, "POS", null,
                                LocalDateTime.now().withHour(10)))))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.title").value("Validation Failed"))
                .andExpect(jsonPath("$.detail").value(containsString("Account ID must not exceed 100 characters")));
    }
